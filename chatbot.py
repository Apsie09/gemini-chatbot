...
class ChatBot:
    """ChatBot class to interact with the GenAI chatbot model."""   
    ...
    def preload_conversation(self, conversation_history = None):
        if isinstance(conversation_history, list):
            ...
            self._conversation_history = [
                self._construct_message('From now on, return the output as JSON object that can be loaded in Python with the key as \'text\'. For example, {"text": "<output goes here>"}', role='user'),
                self._construct_message('{"text": "Sure i can return the output as regular JSON object with the key as text. Here is and example: {"text": "Your Output"}', role='model')
            ]
